...
from util.warmup_schedule import WarmupMultiStepLR


def get_args_parser():
    parser = argparse.ArgumentParser('SAP-DETR', add_help=False)
    ...
    # Model components
    parser.add_argument('--keep_query_pos', default=False, action='store_true', 
                        help="keep query add operation")
    parser.add_argument('--newconvinit', default=False, action='store_true', 
                        help="bilinar conv before initlization")
    parser.add_argument('--sdg', default=False, action='store_true',
                        help="add side directed gaussian map for cross attention map before softmax operation")
    parser.add_argument('--class_embed_diff_each_layer', default=False, action='store_true',
                        help="different bbox prediction head in each layer")      
    
    # Model parameters
    parser.add_argument('--modelname', '-m', type=str, required=True, choices=['sap_detr'])
    ...
    # * Transformer
    parser.add_argument('--bbox_embed_diff_each_layer', action='store_true',
                        help="different bbox prediction head in each layer")          
    parser.add_argument('--rm_self_attn_decoder', action='store_true',
                        help="add rp bias for cross attention map before softmax operation")                  
    ...
    parser.add_argument('--meshgrid_refpoints_xy', action='store_true', 
                        help="Random init the x,y of anchor boxes and freeze them.")
    ...
    parser.add_argument('--set_cost_obj', default=2, type=float,
                        help="Objectness coefficient in the matching cost")
    
    # * Loss coefficients
    parser.add_argument('--inner_loss_coef', default=2, type=float, 
                        help="loss coefficient for inner")
    ...
    parser.add_argument('--warmup_iters', default=0, type=int, 
                        help='number of iteration in first epoch for warm up')
    parser.add_argument('--cache_mode', default=False, action='store_true', help='whether to cache images on memory')
    return parser


def build_model_main(args):
    if args.modelname.lower() == 'sap_detr':
        model, criterion, postprocessors = build_SAPDETR(args)
    else:
...
def main(args):
    ...
    if args.lr_drop:
        if args.warmup_iters:
            lr_scheduler = WarmupMultiStepLR(optimizer, [args.lr_drop*len(data_loader_train)], args.warmup_iters)  # for each iters
        else: 
            lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.lr_drop*len(data_loader_train), gamma=0.1)
    ...
    if args.eval:
        ...
        test_stats, coco_evaluator = evaluate(model, criterion, postprocessors,
                                              ...
                                              args.start_epoch, wo_class_error=wo_class_error, args=args)
        if args.output_dir:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats, coco_evaluator = evaluate(
            ...
            epoch, wo_class_error=wo_class_error, args=args, logger=(logger if args.save_log else None)
        )
